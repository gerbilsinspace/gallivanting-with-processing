...
class Landscape {
  ...
  int sizeOfCell;           // size of each cell
  int w, h;           // width and height of landscape
  int rows, cols;    // number of rows and columns
  float zoff = 0.0;  // perlin noise argument
  float[][] z;       // using an array to store all the height values 

  Landscape(int sizeOfCell_, int w_, int h_) {
    sizeOfCell = sizeOfCell_;
    w = w_;
    h = h_;
    cols = w / sizeOfCell;
    rows = h / sizeOfCell;
  ...
  void render() {
    ...
    for (int x = 0; x < z.length-1; x++)
    {
      for (int y = 0; y < z[x].length-1; y++)
      {
        ...
        pushMatrix();
        beginShape(QUADS);
        translate(x * sizeOfCell - w / 2, y * sizeOfCell - h / 2, 0);
        vertex(0, 0, z[x][y]);
        vertex(sizeOfCell, 0, z[x+1][y]);
        vertex(sizeOfCell, sizeOfCell, z[x+1][y+1]);
        vertex(0, sizeOfCell, z[x][y+1]);
        endShape();
        popMatrix();
      }
    }
